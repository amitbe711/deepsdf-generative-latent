...
\TODO{one sentence with the headline generator-comparison result}
Along the way we report two findings that dominated the outcome: raw ShapeNet
meshes are not watertight, which silently corrupts the SDF \emph{sign} that
DeepSDF trains on, and once that is repaired, added decoder capacity lowers
training loss while making the extracted geometry worse. In both cases the
training loss improved while the extracted geometry degraded, which is the
practical lesson we draw: on a pipeline that ends in Marching Cubes, point-wise
loss is a weak proxy for surface quality. Synthetic parametric shapes are used
...
ablation. The reason is an earlier observation, made before the mesh repair of
\cref{sec:watertight} was in place: at $N=50$, raising $D$ from $16$ to $32$
\emph{degraded} reconstruction Chamfer rather than improving it, echoing the
decoder-width result of \cref{sec:failure-modes}. Since both capacity axes pointed
the same way, crossing $D$ with every $N$ would largely re-measure one effect three
times. The main sweep is therefore $N \in \{10, 50, 150\}$ at $D=16$ (three cells,
each with all three generators), plus one $D=32$ cell at $N=50$ re-measured under
the repaired pipeline (\cref{sec:ablation}) so the ablation is comparable to the
rest of the results. This keeps the comparison the paper is actually about ---
Gaussian vs.\ GMM vs.\ DDPM as $N$ shrinks --- free of a confounded capacity axis.
...
the frozen codes. The GMM uses $K=5$ components (capped so no component receives
fewer than $5$ codes). The DDPM uses $200$ timesteps with a cosine schedule
...
capacity only fit the wrong target more precisely.
...
\caption{Qualitative ShapeNet results, rendered as shaded solids. Rows:
ground-truth chair, reconstruction from its learned code, then samples from the
Gaussian, GMM, and DDPM priors.}
\label{fig:gallery}
\end{figure*}
...
codes --- rather than the decoder --- were the bottleneck. It also determines
whether a $D$-sweep is worth spending the remaining compute on: if $D=32$ does not
help at the middle $N$, crossing $D$ with every $N$ mostly re-measures the same
effect.
...
\TODO{state reconstruction CD/IoU and the valid-ratio range actually observed}
...
\TODO{compare the three generators on Coverage/MMD/1-NN; say whether the gaps
exceed the spread across $N$, and note any consistent ordering}
...
\TODO{report the 1-NN range and whether any generator approaches $0.5$}
...
\TODO{state which effect dominates between $N=10$ and $N=150$}

\paragraph{Latent dimension.} \Cref{sec:ablation} compares $D=32$ against $D=16$
at $N=50$ under the repaired pipeline.
\TODO{report whether $D=32$ helps, hurts, or is neutral, and reconcile with the
pre-repair observation that motivated fixing $D=16$}
...
\TODO{describe what the samples actually look like per prior --- coherence,
diversity, mode collapse, and any visible difference between GMM and DDPM}
...
\TODO{answer the question in one or two sentences, with the numbers that support
it}

The most transferable lesson is not about the priors at all. Two failures
dominated our results, and both presented as \emph{optimization} problems while
actually being something else: non-watertight input meshes corrupting the SDF
sign that DeepSDF trains on (\cref{sec:watertight}), and added capacity
simultaneously lowering training loss and worsening extracted geometry
(\cref{sec:failure-modes}). In both cases the training loss --- the quantity one
naturally monitors --- moved in the reassuring direction while the artifact we
cared about got worse. On a pipeline that ends in Marching Cubes, point-wise loss
is a weak proxy for the surface, and geometry has to be inspected directly.
